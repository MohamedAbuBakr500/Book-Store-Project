...
if (isset($_POST['update_product'])) {
    ...
    $update_query = mysqli_query($conn, "UPDATE `products` SET name = '$update_name', price = '$update_price' WHERE id = '$update_p_id'") or die(mysqli_error($conn));
    
    $update_image = $_FILES["update_image"]["name"];
    $update_image_tmp_name = $_FILES["update_image"]["tmp_name"];
    $update_image_tmp_size = $_FILES["update_image"]["size"];
    $update_folder = 'uploaded_img/' . $update_image;

    if (!empty($update_image)) {
        if ($update_image_tmp_size > 2000000) {
            $message[] = "image file size is too large";
        } else {
            $update_image_query = mysqli_query($conn, "UPDATE `products` SET image = '$update_image' WHERE id = '$update_p_id'") or die(mysqli_error($conn));
